...
export function MatchStatusCard({ status, message }: { status: MatchStatus; message: string }) {
  const isPlaying = status === "PLAYABLE";
  const m = isPlaying ? map["PLAYABLE"] : map["NOT PLAYABLE"];
  const Icon = m.icon;
  const label = isPlaying ? "Playing" : "Not Playing";
  return (
    <div className={`glass-strong rounded-3xl p-6 md:p-8 ring-2 ${m.ring} relative overflow-hidden`}>
      ...
      <div className="relative flex flex-col md:flex-row md:items-center gap-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h2 className={`font-display text-4xl md:text-5xl font-black ${m.text}`}>{label}</h2>
